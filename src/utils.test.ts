...
test("filter by weight", () => {
  expect(
    filterPokemon(pokemonMock(), "saur", [1000, 9999], [0, 9999]).length
  ).toBe(1);
});

test("filter by height", () => {
  expect(filterPokemon(pokemonMock(), "saur", [0, 9999], [0, 10]).length).toBe(
    1
  );
});

test("sort pokemon ascending by id", () => {
  const result = filterPokemon(pokemonMock(), "", [0, 9999], [0, 9999]);
